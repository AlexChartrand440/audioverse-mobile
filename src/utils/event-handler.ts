...
import TrackPlayer, { Event as PlayerEvent, State as PlayerState, Track } from 'react-native-track-player';
...
async function eventHandler(store: Store, data: Data) {
	...
	const playing = () => {
		...
		interval = setInterval(async () => {
			...
			if (track.contentType !== ContentTypes.sermon || position + bumperDuration < duration) {
				return;
			}
