...
export function downloadsQueueReducer(state = initialState, action: DownloadsQueueActionTypes): DownloadsQueueState {
	switch (action.type) {
		case ADD_TO_DOWNLOADS_QUEUE:
			if (
				!state.queue.some(
					(el: { [key: string]: any }) =>
						el.data.id === action.item.data.id && el.data.bitRate === action.item.data.bitRate
				)
			) {
				return {
					...state,
		...
		case REMOVE_FROM_DOWNLOADS_QUEUE:
			return {
				...
				queue: state.queue.filter(
					(el: { [key: string]: any }) => !(el.data.id === action.item.id && el.data.bitRate === action.item.bitRate)
				),
			};
