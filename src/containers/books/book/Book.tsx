...
import { download, LoadBookType, removeLocalChapter, resetAndPlayTrack } from '../../../actions';
...
const Book: React.FC<Props> = ({ navigation, items, pagination, actions }) => {
		...
		if (item.local) {
			Alert.alert(I18n.t('Are_you_sure_you_want_to_delete_this'), '', [
				...
				},
				{
					text: I18n.t('Yes'),
					onPress: () => {
						actions.removeLocalChapter(item);
					},
				},
			]);
		} else {
