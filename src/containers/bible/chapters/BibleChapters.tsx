...
import { download, LoadBibleChaptersType, removeLocalBibleChapter, resetAndPlayTrack } from '../../../actions';
...
const BibleChapters: React.FC<Props> = ({ navigation, items, pagination, bible, actions }) => {
	useEffect(() => {
		if (!items.length) {
		...
		if (item.local) {
			Alert.alert(I18n.t('Are_you_sure_you_want_to_delete_this'), '', [
				...
				},
				{
					text: I18n.t('Yes'),
					onPress: () => {
						actions.removeLocalBibleChapter(item);
					},
				},
			]);
		} else {
