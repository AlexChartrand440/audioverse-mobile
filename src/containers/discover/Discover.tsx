...
const Discover: React.FC<Props> = ({ navigation, history, actions, language }) => {
	...
	useEffect(() => {
		const fetchPosts = async () => {
			const { result } = await fetchGraphQLData(
				Queries.featuredBlogPosts,
				{ language: LANGUAGE_MAP[language] },
				(results) => results.featuredBlogPosts
			);
			if (result.length) {
				setPosts(result);
			}
	...
	const renderList = (data: Track[], title: string) => {
		...
		return (
			<View style={styles.listContainer}>
				...
				<FlatList horizontal={true} data={data} renderItem={renderItem} keyExtractor={(item) => item.id} />
			</View>
		);
	};
	...
	const renderItem: ListRenderItem<Item> = ({ item }) => {
		const artwork =
			typeof item.artwork === 'string' || item.contentType === ContentTypes.sermon
		...
		return <Entry data={data} onPress={() => actions.resetAndPlayTrack([item])} />;
	};
	...
	const renderItemWithParallax: ListRenderItem<Item> = ({ item }) => {
		...
		return (
			<SliderEntry data={data} onPress={() => navigation.navigate({ routeName: 'Post', params: { url: item.id } })} />
		);
	};
	...
	return (
		<View style={styles.container}>
			<ScrollView style={styles.scrollview} scrollEventThrottle={200} directionalLockEnabled={true}>
				...
				<InViewPort disabled={!navigation.isFocused() || wasStoriesVisible} onChange={loadStories}>
					{renderList(stories, 'stories')}
				</InViewPort>
				<InViewPort disabled={!navigation.isFocused() || wasSongsVisible} onChange={loadSongs}>
