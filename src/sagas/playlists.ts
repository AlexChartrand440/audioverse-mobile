...
function* syncLocalToServer(user: UserState) {
	...
	yield all(
		local.map((p) =>
			call(api.fetchGraphQLData, Queries.playlistAdd, p, (results) => ({ nodes: results.playlistAdd }), user)
		)
	);
}
	...
	yield all(
		deleted.map((p) =>
			call(
				...
				Queries.playlistDelete,
				{ id: p.id },
				(results) => ({ nodes: results.playlistDelete }),
				user
			)
		)
	);
...
export function* deletePlaylistItems(playlistsIds: PlaylistsIds) {
	...
	const playlistsItems = allPlaylistsItems.filter(
		(el) => !Object.prototype.hasOwnProperty.call(playlistsIds, el.playlistId)
	);
	...
	if (isConnected && user) {
		try {
			...
			const { result } = yield call(
				api.fetchGraphQLData,
				Queries.userPlaylists,
				{ language: LANGUAGE_MAP[language] },
				(results) => results.me.user.playlists,
				user
			);
			// add to the store
	...
	if (isConnected) {
		try {
			...
			const { result } = yield call(
				...
				Queries.playlistAdd,
				playlist,
				(results) => ({ nodes: results.playlistAdd }),
				user
			);
			if (result) {
