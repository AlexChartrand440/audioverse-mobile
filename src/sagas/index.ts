...
function* rootSaga() {
	yield all([
		...
		takeLatest(CHANGE_LANGUAGE, settings.changeLanguage),
		takeLatest(LOG_OUT, settings.logOut),
		...
		takeLatest(actions.REMOVE_LOCAL_BIBLE_CHAPTER, api.removeLocalBibleChapter),
		takeLatest(actions.LOAD_BIBLE_VERSES, api.loadBibleVerses),
		...
		takeLatest(actions.REMOVE_LOCAL_CHAPTER, api.removeLocalChapter),
		...
		takeLatest(SET_BIBLE_VERSION, bible.setBibleVersion),
	]);
}
