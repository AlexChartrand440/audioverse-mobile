...
export const App: React.FC<Props> = ({ store, persistor }) => {
	...
	const openURL = async (url: string) => {
		if (url.match('/sermons/recordings/')) {
			// recording
		...
		if (url.match('/music/browse')) {
			// music
			routeName = 'ScriptureSongs';
		} else if (url.match(/audiobibles\/books\/\w+\/\w+\/\w+\/\d+/)) {
			// Bible
			routeName = 'Chapters';
		...
		} else if (url.match('/audiobooks/books/')) {
			// book
			routeName = 'Book';
			const urlMatch = url.match(/\d+/) || [];
			id = urlMatch[0];
